    ...
    for alg in algorithms:
        for i in range(1, 5):  # Assuming 4 runs for each algorithm
            ...
            if os.path.exists(log_file):
                ...
                if 'frame' in data.columns and 'extrinsic_reward' in data.columns:
                    data = data[['frame', 'extrinsic_reward']].dropna()
                    data['frame'] = pd.to_numeric(data['frame'], errors='coerce')
                    data['extrinsic_reward'] = pd.to_numeric(data['extrinsic_reward'], errors='coerce')
                    data = data.dropna()
                    all_data[alg].append(data)
                else:
                    print(f"Warning: Missing 'frame' or 'extrinsic_reward' in {log_file}")
...
def plot_training_curves(base_dir, env_name, success_threshold):
    ...
    for alg, color, label in zip(algorithms, colors, labels):
        if len(all_data[alg]) > 0:
            ...
            x_new, y_new = interpolate_data(combined_data_grouped, 'frame', 'extrinsic_reward')
    ...
    plt.savefig(f'{env_name}_Extrinsic_reward.png')
    plt.show()


base_dir = 'exp_fixed/cartpole_swingup_sparse'
env_name = 'Cartpole_Swingup_Sparse'

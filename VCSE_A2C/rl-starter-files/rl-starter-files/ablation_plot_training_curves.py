...
def read_and_concatenate_csvs(base_dir, algorithms):
    ...
    for alg in algorithms:
        for i in range(1, 5):  # Assuming 4 runs for each algorithm
            run_dir = os.path.join(base_dir, f'MiniGrid-DoorKey-8x8-v0-{alg}-{i}')
...
def plot_training_curves(base_dir, env_name, success_threshold):
    algorithms = ['sent', 'vcse', 'vae-vcse', 'renyi-vcse']
    ...
    plt.savefig(f'{env_name}_ablation_curves.png')
    plt.show()


base_dir = 'storage/Ablation'

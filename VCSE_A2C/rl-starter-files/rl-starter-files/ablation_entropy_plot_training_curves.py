...
def read_and_concatenate_csvs(base_dir, algorithms):
    ...
    for alg in algorithms:
        for i in range(1, 5):  # Assuming 4 runs for each algorithm
            run_dir = os.path.join(base_dir, f'MiniGrid-Unlock-v0-{alg}-{i}')
...
def plot_training_curves(base_dir, env_name, success_threshold):
    algorithms = ['SE', 'renyi', 'JER']
    colors = ['blue', 'green','red']
    labels = ['rVCSE', 'rVCRenyi', 'AJER']
    ...
    plt.savefig(f'{env_name}_ablation_entropy.png')
    plt.show()


base_dir = 'storage/Ablation_Entropy/Unlock'
env_name = 'Unlock'
